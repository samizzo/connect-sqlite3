...
module.exports = function(connect) {
    ...
    */
    ...
    function SQLiteStore(options) {
        ...
        this.dbName = options.dbName || this.table;
        var dbPath;

        if(this.dbName !== ':memory:') {
            dbPath = (options.dir || '.') + '/' + this.dbName + '.db';
        } else {
            dbPath = this.dbName;
        }

        this.db = options.dbInstance || new sqlite3.Database(dbPath);
        this.client = new events.EventEmitter();
        var self = this;
        var cleanupInterval = options.cleanupInterval || oneDay;
        ...
        this.db.exec('CREATE TABLE IF NOT EXISTS ' + this.table + ' (' + 'sid PRIMARY KEY, ' + 'expired, sess)',
            function(err) {
                ...
                dbCleanup(self);
                setInterval(dbCleanup, cleanupInterval, self).unref();
            }
        );
    }
  ...
  SQLiteStore.prototype.set = function(sid, sess, fn) {
    ...
    */
    ...
    /**
    * Return an array of all sessions in the database that haven't expired.
    * 
    * @param {function} fn
    * @public
    */
    SQLiteStore.prototype.all = function(fn) {
        ...
        var now = new Date().getTime();
        this.db.all('SELECT * FROM ' + this.table + ' WHERE ? <= expired', now, function (err, rows) {
            if (err) return fn(err);
            var sessions = [];
            for (var i = 0; i < rows.length; i++) {
                sessions.push(JSON.parse(rows[i].sess));
            }
            fn(null, sessions);
        });
    }
